...
Following \citet{Gouskova.etal2015}, three diminutives were formed for each stimulus by suffixing them with \ok{}, \ik{}, and \chik{}. The constructed diminutives included a number of alternations of stem-final consonants described in \sectref{sec:corpdata}: [l] and [ts] palatalize to [lʲ] and [tʃʲ] before \chik{} and \ok{}, respectively: for example, [lumkál] has the diminutive [lumkálʲtʃʲik] and [xakóts] has the diminutive [xakótʃʲik] (alongside [xakótstʃʲik], its \chik-diminutive). Dorsals also palatalize before \ok{}, so [tʲanóx] has the diminutive [tʲanoʂók]. Unlike \citet{Gouskova.etal2015}, dorsals in my experiment also palatalize before \ik{}, reflecting the corpus patterns (see \sectref{sec:corpdata}): thus, [tʲanóx] also has the diminutive [tʲanóʂɨk].
...
% My analysis of the nonce word behavior in \sectref{sec:coranal} requires two pieces of theoretical machinery that have faced criticism: diacritic features and constraints that capture arbitrary statistical patterns in the lexicon. In this section, I discuss alternatives that avoid one or both of these constructs. I conclude that the analysis in \sectref{sec:coranal} is superior for empirical and theoretical reasons.
...
% In the system of \citet{Becker2009}, as in \sectref{sec:coranal} here, assigning a plural to nonce words with [o] in the stem is a matter of assigning one of the features [O] or [I]; the likelihood of a nonce word being given [I] (and thus plural [im]) or [O] (and thus plural [ot]) reflects the relative numbers of nouns with [I] and [O] in the lexicon. This is undoubtedly more grounded than the proposal in \sectref{sec:coranal}, in which speakers can learn arbitrary patterns that do not interact with the phonological grammar. It also requires that generalizations be \emph{product-oriented} \citep[see][]{Bybee1995,Gouskova.etal2015} -- that is, they reflect properties of the inflected (plural) form, not the base (singular); the generalizations in \sectref{sec:coranal} are instead \emph{source-oriented}, in that they reflect properties of the base (really, its underlying form).
