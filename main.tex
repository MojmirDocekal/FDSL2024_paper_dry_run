...
% \author{Guy Tabachnick\orcid{0009-0006-5705-516X}\affiliation{University of Nova Gorica}
\author{}
...
\section{Introduction}\label{sec:intro}ABC
...
The three diminutives discussed in this paper are shown in \tref{dimex}.\footnote{Unless otherwise noted, I ignore the reduction of unstressed vowels.}
...
& `novel' & `package' & `staff'\\
\midrule
% román & román-tʃʲik & `novel'\\
% pakʲét & pakʲét-ʲik & `package'\\
% pósox & posoʂ-ók & `staff'\\
base & román & pakʲét & pósox\\
diminutive & romántʃʲik & pakʲétʲik & posoʂók\\
\lspbottomrule
% \end{tabularx}
\end{tabular}
...
&& `bus' & `pencil' & `hair'\\
\midrule
\SG{} & \NOM{} & avtóbus & karandáʂ & vólos\\
\SG{} & \DAT{} & avtóbusu & karandaʂú & vólosu\\
\PL{} & \NOM{} & avtóbusɨ & karandaʂɨ́ & vólosɨ\\
\PL{} & \DAT{} & avtóbusam & karandaʂám & volosám\\[.5ex]
...
I look at nouns from the main masculine inflection class according to inflectional data from \citet{Zaliznjak1977}, which specifies endings and stress patterns. I exclude words with more than one listed inflectional pattern or stress location, words listed as mass nouns (which have no plural), and words with one of three stem extensions: plural [j] (e.g.\ [klʲin] `wedge', nominative plural [klʲínʲja]), singular [ʲin] (e.g. [ɡraʐdanʲín], nominative plural [ɡráʐdanʲe]), and singular [ʲón(o)k] alternating with plural [ʲát] (e.g. [tʲelʲónok] `calf', nominative plural [tʲelʲáta].

I then constructed possible diminutives for each noun, taking into account regular and semi-regular morphophonological alternations. Thus, as described in \sref{dimbg}, \ok{} palatalizes velars, so [posoʂók] was included as a possible diminutive of [pósox] `staff' but [posoxók] was not. Nouns ending in velars almost never take \ik{}, so it is unclear whether this diminutive suffix triggers velar palatalization as well \citep[see][]{Kapatsinski2010,Magomedova.Slioussar2017}; in my data, however, potential diminutives with \ik{} after non-alternating velars were false positives (e.g.\ [lóɡʲik] `logic' is not the diminutive of [loɡ] `ravine'), so I likewise only included potential \ik{} diminutives with palatalized velars: [pósoʂɨk], not [pósoxik].\footnote{Although it is itself the outcome of a palatalization alternation, Russian [ʂ] patterns as a hard (unpalatalized) consonant phonologically, meaning that /i/ following [ʂ] surfaces as [ɨ].} This diminutive also typically causes an alternation of [ts] to [tʃʲ], so a word like [abzáts] `indentation' had potential diminutives [abzátsɨk] and [abzátʃʲik] (exceptionally, in this case the diminutive is attested with [ts]). Similarly, \chik{}, as a rule, palatalizes [l], so [orʲólʲtʃʲik] was tested as a diminutive of [orʲól] `eagle' alongside [orʲóltʃʲik] (forms like the latter appear very rarely, if ever). Finally, \ik{} often but not always triggers a vowel--zero alternation for words that have one in their inflectional pattern. Thus, [xrʲebʲét] `ridge' (genitive singular [xrʲebʲtá]) has potential diminutives [xrʲebʲétʲik] and [xrʲébʲtʲik] (both of which are attested).
...
While the Russian National Corpus is much larger and better tagged than the source of data used by \citet{Gouskova.etal2015} in their corpus study, it does have two main sources of false positive diminutives. The first is that forms are sometimes mistagged -- accordingly, I manually filtered out tokens whose endings did not match the expected case endings. Second, there are ``pseudo-diminutives'' that are orthographically identical to expected combinations of base and diminutive but do not have the intended meaning. The main source of these is homographic suffixes, such as in [alkoɡólʲik] `alcoholic', which is not the diminutive of [alkoɡólʲ] `alcohol', and [pulʲemʲóttʃʲik] `machine gun operator', which is not the diminutive of [pulʲemʲót] `machine gun' \citep[cf.][]{Gouskova.etal2015,GuzmanNaranjo2019}. These suffixes tend to derive animate nouns from inanimate bases, so I removed any diminutives tagged in the corpus with an animacy feature differing from that of its base in the dictionary. More problematic are words ending in \emph{unstressed} [ok], a suffix with numerous uses including an unproductive diminutive (this suffix is written the same as \ok{} in Russian orthography, which does not typically mark stress). For example, the verb [obrʲézatʲ] `trim' has derivatives including [obrʲéz] `edge' and [obrʲézok] `trimming' (usually plural, as in English), but the latter is not the diminutive of the former. In this case, I removed potential diminutives that were themselves listed in \citet{Zaliznjak1977} as ending in unstressed [ok]. Finally, nouns with the suffix [nʲik] (e.g.\ [plavnʲík] `fin') look like diminutives of nouns ending in [(e)nʲ] (e.g.\ [plavʲenʲ] `flux'); I removed all such base--diminutive pairs as well. These measures are altogether fairly effective, though the final data set does include some false positives nonetheless.
...
\tref{corpdesc} shows the distribution of the different diminutives according to the inflectional stress patterns and plurals of their bases. The two nouns with suffix stress in the singular that also take plural \pla{} ([rukáv] `sleeve', which appears with \chik, and [obʂláɡ] `cuff', which does not have a diminutive in the corpus) are listed with other suffix-stress nouns.
\begin{table}
...
\lspbottomrule
\end{tabular}
\caption{Distribution of diminutives in the Russian National Corpus by stress pattern and plural suffix (\unit{percent} [ók] measures the percentage of nouns attested with at least one diminutive that are attested with [ók])} % fix
\label{tab:corpdesc}
\end{table}
...
\tref{corpmod} shows that this correlation between \ok{} and the morphological factors holds even when phonological factors are taken into account: nouns with suffix and mobile stress appear with \ok{} significantly more than those with fixed final stem stress (as do nouns with stress fixed on a non-final syllable); this difference is especially concentrated in nouns with plural \pla, which take \ok{} significantly more than even other nouns with mobile stress.\footnote{Together, the models for \ik{} and \chik{} mentioned in \sref{corphyp} form the mirror image of the model for \ok{} in \tref{corpmod}: nouns with mobile stress are significantly less likely to take \ik{} than nouns with suffix stress, while the other three comparisons (pre-final vs.\ final stem stress, suffix/mobile vs.\ stem stress, and mobile with plural \pla{} vs.\ mobile with plural \pli{}) show a significant negative effect for \chik{} (that is, also in the opposite direction from \ok{}).}
% \begin{table}
...
I selected 87 of the 300~nonce word stimuli used by \citet{Gouskova.etal2015}, which were designed to test the effect of various phonological factors on diminutive selection. All stimuli were disyllabic with final stress, no word-internal hiatus, and no word-final clusters (one stimulus with a cluster was accidentally included in the set; trials with this word were removed). I adopted the previous study's balance by manner of the final consonant (22 approximant, 22 nasal, 21 fricative, 22 plosive). Their plosive category also included a small number of nouns ending in affricates (which they coded as strident stops); one of these stimuli ([xakóts]) is in my final set, coded as an affricate to match the corpus study. \citet{Gouskova.etal2015} verified that the stimuli are phonotactically plausible words of Russian through their native intuition and a grammar output by the UCLA Phonotactic Learner \citet{Hayes.Wilson2008} trained on their corpus of Russian masculine nouns.

Following \citet{Gouskova.etal2015}, three diminutives were formed for each stimulus by suffixing them with \ok{}, \ik{}, and \chik{}. The constructed diminutives included a number of alternations of stem-final consonants described in \sref{corpdata}: [l] and [ts] palatalize to [lʲ] and [tʃʲ] before \chik{} and \ok{}, respectively: for example, [lumkál] has the diminutive [lumkálʲtʃʲik] and [xakóts] has the diminutive [xakótʃʲik] (alongside [xakótstʃʲik], its \chik-diminutive). Dorsals also palatalize before \ok{}, so [tʲanóx] has the diminutive [tʲanoʂók]. Unlike \citet{Gouskova.etal2015}, dorsals in my experiment also palatalize before \ik{}, reflecting the corpus patterns (see \sref{corpdata}): thus, [tʲanóx] also has the diminutive [tʲanóʂɨk].
...
\begin{figure}
 ...
 \begin{tabular}{lll}
  I'm interested in the price of that \textbf{\underline{mimɡolʲ}} & \fbox{play} & [mʲɪmɡólʲ] \\
  I would like to buy those \textbf{\underline{mimɡolʲi}}. & \fbox{play} & [mʲɪmɡɐlʲí]
 \end{tabular}\\[\baselineskip]
 
 \emph{Listen to the audio recordings and select the word written in the sentence above.}\\[\baselineskip]
 
 \begin{tabular}{lll}
  \like{0} mimɡolʲi & \fbox{play} & [mʲɪmɡólʲɪ]\\
  \like{0} mimɡolʲa & \fbox{play} & [mʲɪmɡɐlʲá]\\
  \like{7} mimɡolʲi & \fbox{play} & [mʲɪmɡɐlʲí]\\
 ...
 \begin{tabular}{l @{\hspace{1em}} lll}
  She had a little & \like{0} mimɡolʲik & \fbox{play} & [mʲɪmɡólʲɪk]\\
  & \like{0} mimɡolʲok & \fbox{play} & [mʲɪmɡɐlʲók]\\
  & \like{7} mimɡolʲtʃik & \fbox{play} & [mʲɪmɡólʲtʃʲɪk]\\
 \end{tabular}\\[.5\baselineskip-\fboxsep]
 }}\vspace{-.5\fboxsep}
 \fbox{\parbox{.97\textwidth}{
 \centering\hfill\\[.5\baselineskip-2\fboxsep] 
...
\tref{expdesc} shows how often participants selected each diminutive for nonce words depending on the plural with which they were presented.
\begin{table}
...
As \tref{expmod} shows, the experimental effect of plural is significant, even when other factors are taken into account: plural \pla{} conditioned selection of \ok{} more frequently than plural stressed \plsi, which in turn saw significantly more use of \ok{} than its unstressed counterpart.
\begin{table}
\begin{tabular}{l@{~~}l@{~~}l@{}S@{~~~}S[table-format = 0.2, print-zero-integer = false]@{~~~}S@{~~~}S[table-format = +0.4, print-zero-integer = false]@{~}l}
\lsptoprule
\multicolumn{3}{l}{Effect} & {Estimate} & {SE} & {$z$ value} & {$p$ value}\\
...
We are interested in the relationship between three morphological properties: stress pattern, plural allomorphy, and diminutive allomorph. How is each represented? As described in \sref{strbg}, nouns with fixed stem stress are typically analyzed as having underlying lexical accent on the stressed syllable (e.g.\ /avtóbus/), fixed suffix stress can be analyzed as a floating lexical accent (e.g.\ /karandaʂ´/), and mobile-stress nouns are underlyingly accentless (e.g.\ /volos/).

In \sref{plbg}, I noted that masculine nouns with plural \pla{}, which almost always have mobile stress in that they have stem stress in the singular and suffix stress in the plural, cannot all be analyzed as underlyingly accentless because the stem stress can fall on medial syllables (as in [utʃʲítʲelʲ] `teacher', whose plural is [utʃʲitʲelʲá]), whereas Russian phonology predicts that mobile-stress nouns should have stress on the first syllable in the singular. Thus, they may have a different underlying accent than mobile nouns with plural \pli. We will leave this problem aside and focus on the fact that these nouns select a different plural allomorph than most nouns of their inflectional macroclass.\footnote{One possibility, inspired by \citet{Matushansky2024}, is that plural \pla{} nouns take a plural suffix that is segmentally empty but accentually dominant, meaning that it erases any stress on its base. Thus, such nouns would behave as though accentless in the plural, meaning that the plural suffixes, which are underlyingly accented (including nominative /á/), would be stressed.} Since these nouns do not seem to differ syntactically from normal masculine nouns \citep{Matushansky2025}, I assume that they bear an inflection class feature, \feat{pl-a}, which resides in their phonological underlying form \citep{Harley.TubinoBlanco2013,Gouskova.Bobaljik2022,Tabachnick2023}: % cite pages lol
\urfeat{utʃʲítʲelʲ}{pl-a}.

Similarly, a noun's choice of diminutive must be memorized (for many nouns, at least). This, too, is typical behavior of arbitrary selection handled by diacritic features \feat{ok}, \feat{ik}, and \feat{chik} -- since the diminutive of [román] `novel' is [romántʃʲik], its underlying form is \urfeat{román}{chik}; the underlying form of [pósox] `staff', whose diminutive is [posoʂók], is \urfeat{pósox}{ok}. Since \ok{} diminutives have fixed suffix stress and are stress-dominant (erase any stress marks on their base), the underlying form is \urfeat[]{\underline{o}k´}{$+$dominant}, where /\underline{o}/ represents the fleeting vowel that disappears upon further suffixation and [$+$dominant] tags affixes that show stress dominance (see \sref{dimbg}; the exact phonology of these two properties is not relevant).

\subsection{Correlations between pieces of underlying structure}\label{sec:coranal}
I propose that speakers' have learned generalizations of varying strengths that determine the well-formedness of underlying forms -- in essence, gradient morpheme structure constraints \citep[see][]{Booij2011} -- represented formally in \eref{consts}:
\ea\label{ex:consts} \emph{Russian gradient morpheme structure constraints}\\
    ...
    \z
\z
The first constraint, \eref{constsyll}, is a cooccurrence restriction violated by underlying forms that contain both an accented syllable and the diacritic feature \feat{ok}.\footnote{This restriction is presented as non-local; however, if -- as assumed by e.g.\ \citet[][374]{Chomsky.Halle1968} -- diacritic features are distributed to every segment of a lexical item, \eref{constsyll} can be represented as a local cooccurrence restriction.} Given that fixed stem stress is represented by lexical accent on a syllable, \eref{constsyll} captures the fact that nouns with stem stress are substantially underrepresented among words that take \ok{} relative to words with suffix or mobile stress. Since such nouns do still appear with some frequency, however, this constraint would not be overwhelmingly strong. % lay out what ``strength'' means?

The second constraint, \eref{constpd}, is an implicational relationship between two diacritic features \citep[cf.][]{Halle.Marantz2008}: lexical items that have the \feat{pl-a} feature (indexing plural \pla) must also have the \feat{ok} feature (indexing diminutive \ok).\footnote{Since the \feat{ok}, \feat{chik}, and \feat{ik} features are essentially complementary, \eref{constpd} can be replaced by two cooccurrence restrictions penalizing plural \pla{} nouns with other diminutives: *\feat{pl-a}, \feat{ik} and *\feat{pl-a}, \feat{chik}. This is theoretically desirable under approaches \citep[e.g.][]{Hayes.Wilson2008} that require constraints to penalize the presence of structure -- that is, be framed negatively.} % cite
...
The constraints in \eref{consts} are \emph{not} a part of the generative grammar; they are not invoked in the course of a normal derivation. Instead, as well-formedness constraints on underlying forms, they are invoked when necessary to ``fill in'' an underlying form \citep[cf.][]{Gouskova.etal2015} that is lacking crucial information, as in the case of nonce words. % PCFP
The task of choosing a diminutive for a novel word (or any word for which a speaker has no stored diminutive) is as follows. I assume that abstract syntactic objects like a diminutive head are spelled out phonologically using rules of realization, as in Distributed Morphology \citep[see e.g.][]{Halle.Marantz1993,Harley.Noyer1999,Steriopolo2008}. The Russian speaker's set of realization rules includes (among others) the three possibilities for the diminutive in \eref{dimvis} and two for the plural in \eref{plvis} (the latter of which are considerably oversimplified in ways that are not relevant here):\footnote{In contexts with \feat{pl-a}, the more specific rule in \eref{avi} wins over the generic rule in \eref{ivi} according to the Subset Principle \citep{Halle1997b}. In other contexts, the default \eref{ivi} wins.}
...
\ex\label{ex:plvis} \emph{Rules of realization for the Russian plural (oversimplified)}\\
    ...
    \z
\z
The rules in \eref{dimvis} insert each of the three diminutives in the context of its corresponding diacritic feature, which is brought into the derivation by the noun stem. Thus, the diminutive will be spelled out as /tʃʲik/ in the context of \urfeat{román}{chik} because the \feat{chik} feature matches the structural conditions for rule \eref{chikvi}. If a noun does not have one of the three features, none of the rules in \eref{dimvis} can apply. This, of course, is the situation with nonce words, for which speakers have not previously seen a diminutive and thus cannot have a stored selectional preference. Thus, if they wish to assign a diminutive to a noun, they must place one of the diminutive features on its lexical item.
...
each of the three possible options is assigned a ``well-formedness'' score, and the better a feature for a given underlying form, the more likely it is to be assigned. The well-formedness score is assigned by gradient, weighted constraints, including those in \eref{consts}. Of course, by this point the speaker does already have an underlying form for the nonce word inferred from the singular and plural, as shown in \tref{nonceurs}.
\begin{table}
\begin{tabular}{lll}
\lsptoprule
Singular & Plural & Underlying form\\
\midrule
{[mʲimɡólʲ]} & [mʲimɡólʲi] & /mʲimɡólʲ/\\
{[mʲimɡólʲ]} & [mʲimɡolʲí] & /mʲimɡolʲ´/\\
{[mʲimɡólʲ]} & [mʲimɡolʲá] & \urfeat{mʲimɡolʲ´}{pl-a}, \urfeat{mʲimɡólʲ}{pl-a}\\
\lspbottomrule
\end{tabular}
\caption{Possible underlying forms for the nonce word [mʲimɡólʲ] given observed singular and plural forms}
\label{tab:nonceurs}
\end{table}
This table shows how a word's singular and plural indirectly affect its likely diminutive. If a word is shown with unstressed plural \pli, it must have fixed stem stress and thus underlying lexical accent on the syllable. Assigning \feat{ok} to this word would violate \eref{constsyll}, so the likelihood of doing so will be lower than for words without an underlying accented syllable. Nouns with stressed plural \plsi{} must have suffix stress, and thus get a floating stress mark; assigning \feat{ok} to such words would not be penalized by \eref{constsyll}. If a noun has plural \pla, it gets the \feat{pl-a} feature, so assigning anything other than \feat{ok} to this word (i.e., \feat{ik} or \feat{chik}) would be heavily penalized by \eref{constpd}. Thus, words with \feat{pl-a} are more likely than others to get \feat{ok}. Since nouns with plural \pla{} can have different stress patterns in their singular and plural (see \sref{fanal}), the nominative singular and plural alone are compatible with two stress patterns in the singular: fixed suffix stress, with floating underlying accent (which would fall on the last syllable of the stem in the unsuffixed nominative), and fixed stem stress, with linked underlying accent -- these nonce words cannot be mobile stress nouns, because in that case they would have the first syllable stressed in the nominative singular. If speakers represent these nouns as having fixed suffix stress, assigning \feat{ok} to them would violate \eref{constsyll}; this may be one reason why the experimental effect of plural \pla{} was relatively small.
...
%     \z
% \z
This move introduces two further issues. First, the \feat{ok} feature is still needed for the nouns with diminutive \ok{} and plural \pli{} (which far outnumber those with plural \pla). This feature is, as before, invoked in rule \eref{okvi}; thus, two rules point to the same realization, \urfeat[]{\underline{o}k´}{$+$dominant} (or, alternatively, we combine them into a single rule with a disjoint context).
Second, we need an additional feature, \feat{ik+pl-a}, to override the association between \feat{pl-a} and diminutive \ok{}. This rule sits alongside \feat{pl-a} for those few words with plural \pla{} that take a different diminutive (\ik). Both features are then invoked in the diminutive spellout rule in \eref{ikavia}:
...
\z
This rule duplicates spellout of /ʲik/ from the rule in \eref{ikvi} just as the rule in \eref{okavia} duplicates \eref{okvi}.\footnote{These nouns must also have the \feat{pl-a} feature to match rule \eref{avi} spelling out the plural as \pla. The rule in \eref{ikavia} must then require both \feat{pl-a} and \feat{ik+pl-a} in order to be more specific than rule \eref{okavia}, and thus win out by the Subset Principle \citep{Halle1997b} when both rules' conditions are met.}

% We can simplify this analysis slightly if we assume that diacritic features may be in an inheritance relation -- that is, \feat{pl-a} is a sub-feature of \feat{ok}, while \feat{ik+pl-a} is a sub-feature of \feat{ik}. In this case, the rules in \eref{dimvias} are not necessary, since the \feat{pl-a} feature will count as an \feat{ok} feature in the context of the usual rule spelling out the diminutive as \ok{} in \eref{okvi}. However, if diacritic features are, as proposed in this section, stand-ins for features or structures that properly use the phonological (or syntactic) vocabulary, it is difficult to see how this inheritance structure would work. % rework

Since nouns with plural \pla{} get \feat{pl-a}, rule \eref{okavia} should apply by default to these words without the need for productive feature filling as described in \sref{coranal}. In this case, we should expect the effect size of plural \pla{} to be much greater in the nonce word study than it was: even accounting for some amount of random noise in the task effect, the use of \ok{} for nouns with plural \pla{} is predicted to be close to ceiling. In actuality, \ok{} was selected for nouns with \pla{} just over half the time -- only a moderately higher rate than for nouns with plural \pli.

% Alternately, speakers may (by some mechanism similar to that described in \sref{coranal}) choose proportionately between assigning \feat{pl-a} and \feat{ik+pl-a}. This predicts similar empirical results to the analysis in \sref{coranal}, although it more strongly predicts that nouns with plural \pla{} should never be assigned \chik{, since they are not in the lexicon; in the nonce word study, however, such words were assigned \ik{} and \chik{} at roughly equal rates.

In an attempt to reduce theoretical machinery, we have instead increased it, with one new feature, \feat{ik+pl-a}, and two new rules, in \eref{okavia} and \eref{ikavia}. Doing so also reduced our empirical coverage. Thus, plural and diminutive allomorph selection should remain \emph{formally distinct} and for the correlation between them to be handled in a separate pattern-matching module.

This finding has implications for morphological theory. While the analysis presented here uses diacritic features, some linguists argue against the use of morphological diacritics on theoretical grounds, aiming to replace them either with complex stored stems \citep[e.g.][]{BermudezOtero2013} or pieces of phonological \citep[e.g.][]{Lampitelli.Ulfsbjorninn2023} or syntactic \citep[e.g.][]{Caha2021} structure. This section, then, shows that any analyses of Russian without diacritics must be flexible enough to satisfy the following two criteria: first, a noun's plural and diminutive selection are stored as \emph{separate} and \emph{independent} markers, and second, all three morphological markers must be accessible to pattern-matching mechanisms like the morpheme structure constraints proposed in \sref{coranal}.
...
% My analysis of the nonce word behavior in \sref{coranal} requires two pieces of theoretical machinery that have faced criticism: diacritic features and constraints that capture arbitrary statistical patterns in the lexicon. In this section, I discuss alternatives that avoid one or both of these constructs. I conclude that the analysis in \sref{coranal} is superior for empirical and theoretical reasons.

% \subsubsection{Phonologically ``natural'' generalizations}
% The gradient morpheme structure constraints proposed above are arbitrary: they are learned from patterns in the data, but do not reflect universal preferences for well-formedness (all the more so because they do not evaluate surface forms). They are also divorced from the generative grammar in the sense that they reside in a pattern-matching module separate from the morphological interface that spells out syntactic material phonologically and the phonological module that derives surface forms from underlying forms.

% \citet{Becker2009} and \citet{Becker.etal2011} view such ungrounded pattern learning with skepticism. % cite others
% Instead, they argue that speakers do not learn all patterns from their data, but only those that are phonologically ``natural'' -- that is, that they can be expressed in terms of constraints that are plausibly part of a universal set. Thus, the two constraints proposed in \eref{consts} must be translated into constraints that prefer \ok{} to \ik{} and \chik{} for words with suffix and mobile stress and for words with plural \pla{}, respectively. As pointed out previously, what \ok{} has in common with nouns with these nouns is stress on the suffix. This can be captured with the constraint \textsc{NoFlop-OO} \citep[cf.][]{Alderete2001}. \textsc{NoFlop} is a faithfulness constraint violated when two forms have stress on different syllables; OO represents the fact that this is a transderivational (output--output) constraint comparing multiple forms derived from the same base -- in this case, the diminutive and at least one form with an inflectional suffix. Let us assume that speakers identify one useful form as the base for comparison \citep[see][]{Albright2002} -- for the sake of concreteness, this is the dative plural (which bears stress for the mobile stress nouns under study). This constraint will penalize a diminutive form that has a different syllable stressed from the dative plural -- concretely, if the dative plural has stem stress, forms with \ok{} are penalized but not those with \ik{} or \chik, and if the dative plural has suffix stress, forms with \ik{} or \chik{} are penalized but not those with \ok.
% % NoFlop-IO can lower the rates of -ok among stem stress nouns, but is that enough? Otherwise the baseline for -ok would have to be much higher than expected, and this goes against B 2009's (somewhat sheepish) way of figuring out which constraint clone pair is the most salient for the coin flip

% Like the analysis described in \sref{coranal}, for \citet{Becker2009}, speakers roll a weighted die to assign a diacritic feature to new stems that indexes one of the diminutive suffix. However, in his theory, these features encode the preference in a different way: they index copies of constraints that are ranked in different locations. This requires that allomorph selection take place in the phonology. That is, the input to phonology includes all possible diminutive suffixes, and the choice of diminutive is left to constraints. In this case, words with suffix stress that take \ok, like [ɡrʲib] `mushroom' (diminutive [ɡrʲibók]), are associated with a higher-ranked copy of \textsc{NoFlop-OO}, while words with suffix stress that take a different diminutive suffix, like [stol] `table' (diminutive [stólʲik]), activate a lower copy of the constraint that is ranked below whatever constraint prefers \ik{} for this word.

% \ea\label{ex:otdimok} \emph{Outline of tableau for [ɡrʲib]}\\
%     \begin{tableau}{c|c|c}
%         \inp{/ɡrʲib´\textsubscript{\feat{ok}}-$\left\{\begin{array}{@{}c@{}}\text{ok´}\\\text{ʲik}\\\text{tʃʲik}\end{array}\right\}$}    \const*{\begin{tabular}{@{}c@{}}\textsc{NoFlop-OO}\textsubscript{\feat{ok}}\\compare: \textsc{dat.pl}\end{tabular}}  \const*{\begin{tabular}{@{}c@{}}Other\\constraints\end{tabular}} \const*{\begin{tabular}{@{}c@{}}\textsc{NoFlop-OO}\textsubscript{[\^{}ok]}\\compare: \textsc{dat.pl}\end{tabular}}
%         \cand*[\Optimal]{ɡrʲibók}   \vio{}      \vio{*} \vio{}
%         \cand*{ɡrʲíbʲik}            \vio{*! (vs.\ ɡrʲibám)}    \vio{}  \vio{}
%         \cand*{ɡrʲíbtʃʲik}          \vio{*! (vs.\ ɡrʲibám)}    \vio{}  \vio{}
%     \end{tableau}
% \ex\label{ex:otdimik} \emph{Outline of tableau for [stol]}\\
%     \begin{tableau}{c|c|c}
%         \inp{/stol\textsubscript{\feat{ik}}-$\left\{\begin{array}{@{}c@{}}\text{ok´}\\\text{ʲik}\\\text{tʃʲik}\end{array}\right\}$}    \const*{\begin{tabular}{@{}c@{}}\textsc{NoFlop-OO}\textsubscript{\feat{ok}}\\compare: \textsc{dat.pl}\end{tabular}}  \const*{\begin{tabular}{@{}c@{}}Other\\constraints\end{tabular}} \const*{\begin{tabular}{@{}c@{}}\textsc{NoFlop-OO}\textsubscript{[\^{}ok]}\\compare: \textsc{dat.pl}\end{tabular}}
%         \cand*{stolók}              \vio{}    \vio{*!}  \vio{}
%         \cand*[\Optimal]{stólʲik}   \vio{}    \vio{}    \vio{* (vs.\ stolám)}
%         \cand*{stóltʃʲik}           \vio{}    \vio{*!}  \vio{* (vs.\ stolám)}
%     \end{tableau}
% \z

% As before, when a speaker encounters a new word with suffix stress and wishes to form its diminutive, they must assign it either the \feat{ok} feature or one of the other features (\feat{ik} and \feat{chik}, which we can assume are both ``daughters'' of the feature [\^{}ok]. The die is weighted based on the proportion of words with \feat{ok} and [\^{}ok], respectively: since most words with suffix stress that appear with a diminutive take \ok{} (and thus are associated with the \feat{ok} copy of \textsc{NoFlop-OO}, the likelihood of the \feat{ok} feature being assigned to a new word with suffix stress is relatively high.

% While the above analysis can capture the ``naturalness'' of the affinity of \ok{} for nouns with stress on the suffix through paradigm uniformity, it has several issues. First, any notion of trans-derivational correspondence requires the phonology to send information back to the morphology to output the necessary forms for comparison (in this case, the dative plural). Note that this cannot simply be a matter of \emph{retrieval}: under this model, for the nonce word task, speakers must \emph{construct} a dative plural form that they have not seen before, on the basis of the underlying form gleaned from the nominative singular and plural, in order to compare it to the diminutive.\footnote{This problem would be avoided if the nominative plural was used as a base, but that is likely insufficient for the Russian case, since mobile stress nouns vary in the location of their stress in the nominative plural, as described in \sref{plbg}.} This gives a lot of power to the phonology in violation of common assumptions about modularity, in which information between the word-constructing module and the phonological module only flows in one direction in production. % cite

% Placing allomorph selection in the phonology also leads to some undesirable consequences. First, recent work \citep[e.g.][]{Paster2009,Pak2016,Kalin2020,Kalin2022,Stanton2023} has argued that allomorph selection must \emph{precede} phonology, and while much of this line of work is focused on phonologically conditioned suppletive allomorphy (which Russian diminutive selection is not, at least not fully), many of the arguments presented hold more generally. Second, the particular architecture presented by \citet{Becker2009} is awkward, at best, for the case at hand.

% % is this true? read!
% For \citet{Becker2009}, each salient factor influencing allomorph selection must be associated with a distinct pair of cloned constraints over which statistics are tracked. This includes the near-categorical preference for \ok{} among words ending in velars, in which it also triggers a consonant alternation (as in [pósox] `staff', whose diminutive is [posoʂók]). This generalization is source-oriented, meaning that the relevant factor is the base noun, not the diminutive: when a diminutive contains a \emph{faithful} stem-final [ʂ] (or [tʃʲ ʐ], the results of alternations triggered for [k ɡ] by \ok) corresponding to the same sound in the base, there is no such preference for \ok{} -- if anything, nouns ending in fricatives and affricates were \emph{less} likely to take \ok{} than those ending in plosives in the corpus study. Thus, the relevant constraint in this case cannot be one of markedness. However, the preferred diminutive suffix is the one that \emph{triggers} the alternation.\footnote{\citet{Kapatsinski2010} shows that speakers do sometimes alternate stem-final velars when forming diminutives with \ik, but much less often than they do with \ok.} Thus, however the trigger of the alternation is represented, the relative constraint must involve \emph{anti-faithfulness}. While anti-faithfulness constraints have been proposed \citep{Alderete1999}, their use has typically been limited to base--derivative correspondence (which would otherwise be unnecessary in this case) in particular morphological circumstances, % necessary?
% and they predict a bidirectional alternation: velars should palatalize and vice versa. % be stronger about this?
% % UseListed possible

% While the notion that speakers only learn ``natural'' correlations is intuitively appealing and has some empirical support for more solidly phonological cases, the tools needed to actually enforce this ``naturalness'' lead to some unwelcome consequences in the case of the Russian diminutives. Of course, we have not yet mentioned the correlation between plural \pla{} and \ok, where the most ``natural'' implementation would probably be a transderivational correspondence constraint like \textsc{Ident-OO}-[high] between the diminutive and the nominative plural, which prefers \ok{} for words with plural \pla{} (where neither suffix vowel is high) and \ik{} and \chik{} for words with plural \pli{} (where all suffix vowels are high). This has the added issue of requiring comparison to a different base than \textsc{NoFlop-OO}, where the nominative plural is not appropriate.

% Finally, we can ask whether there is a way to capture the fact that \ok{} increases paradigm uniformity for nouns with suffix stress without the theoretical baggage of the system described above. One possibility is to describe this factor as a target-oriented generalization that is not encoded in the \ldots

% The first ranking is given by \textsc{NoFlop-IO} \citep[cf.][]{Alderete2001}, a faithfulness constraint violated when an accent mark is associated with different syllables in the underlying and surface forms. This constraint is violated by diminutives where \ok{} attaches to a stem with an underlying lexical accent on a syllable -- that is, nouns with fixed stem stress. The correlation between \pla{} and \ok{} is harder to capture in constraints. There is no phonological characteristic shared by all nouns with plural \pla{} and no others, so in deriving diminutive forms, no faithfulness or markedness constraint will favor \ok{} over \ik{} and \chik{} for \emph{all} plural \pla{} stems. I return to the correlation between plural \pla{} and \ok{} in \sref{abstrep}.


% In fact, I have mentioned a product-oriented generalization % xref
% governing the morphological factors correlated with diminutive \ok: suffix stress. Nouns with some or all stressed suffixes, especially those with plural \pla{}, prefer \ok{}, which itself pulls stress from the stem onto the suffix. Thus, these preferences can be viewed as an expression of \emph{paradigm uniformity}, for a loose definition of ``paradigm'': % cite Ackerman
% nouns with suffix stress in some inflected and derived forms also prefer to have suffix stress in others. In the remainder of this section, I describe two possibilities for how to formally implement this effect.

% First, let us assume an approach like that of \citet{Becker2009}: product-oriented generalizations are derived through association with clones of constraints in the phonological grammar. Here, the relevant constraint must be \textsc{NoFlop-OO} \citep[cf.][]{Alderete2001}. \textsc{NoFlop} is a faithfulness constraint violated when two forms have stress on different syllables; OO represents the fact that this is a transderivational (output--output) constraint comparing multiple forms derived from the same base -- in this case, the diminutive and at least one form with an inflectional suffix. Words with mobile or suffix stress that take \ok{} would be associated with \textsc{NoFlop-OO}\textsubscript{\feat{ok}}, a copy of this constraint that is ranked above some other constraint (for example, a constraint enforcing stem stress) that prefers \chik{} and \ik{} diminutives to those with \ok. Thus, when diminutives of these nouns are derived, \textsc{NoFlop-OO}\textsubscript{\feat{ok}} will force selection of \ok{} as the diminutive allomorph (which, under this theory, takes place in the phonology). Nouns that take \chik{} and \ik, on the other hand, would be associated with a lower-ranked copy of \textsc{NoFlop-OO}. Since most nouns in the lexicon with suffix and mobile stress take \ok, and are thus associated with the \feat{ok} feature and its associated copy of \textsc{NoFlop-OO}, new nouns with this stress pattern will likely be given the \feat{ok} feature as well.

% For example, \cite{Becker2009} looks at a pattern in Hebrew nouns: while most, like [alón] `oak', pluralize with the suffix [im] ([aloním]), a sizable minority instead take [ot], which is the usual plural suffix for feminine nouns (like [xalón] `window', plural [xalonót]). Most of these nouns, like [xalon], contain an [o], and Hebrew speakers extend this pattern to nonce words as well: they assign [ot] more often to masculine nonce words with [o] than to those without [o]. % phntit?

% For \citet{Becker2009}, this phonological generalization must be tied to a constraint that prefers [ot] over [im], but only for nouns with [o] in their stem. This constraint is \textsc{Local}(o), which requires any [o] to be licensed either through stress (as in the stem of [xalón] and the suffix of [xalonót]) or through autosegmental association with a stressed [o] in an adjacent syllable -- thus, [aloním] violates \textsc{Local}(o) because the stem vowel is unlicensed. This constraint is cloned \citep[cf.][]{Pater2006}, so that two copies occupy different rankings: the higher copy, indexed [O], is above the constraint preferring [im], which enforces a match between the gender of the noun and the gender of the selected suffix (in his theory, allomorph selection occurs in the phonology, which has access to the morphosyntactic properties of each morpheme). While the lower copy, indexed [I], is below it; nouns with [o] in the stem that pluralize with the expected [im], like [alón], are given the feature [I] (\urfeat{alon}{I}), while those with [o] in the stem and plural [ot] have the feature [O] (\urfeat{xalon}{O}).

% In the system of \citet{Becker2009}, as in \sref{coranal} here, assigning a plural to nonce words with [o] in the stem is a matter of assigning one of the features [O] or [I]; the likelihood of a nonce word being given [I] (and thus plural [im]) or [O] (and thus plural [ot]) reflects the relative numbers of nouns with [I] and [O] in the lexicon. This is undoubtedly more grounded than the proposal in \sref{coranal}, in which speakers can learn arbitrary patterns that do not interact with the phonological grammar. It also requires that generalizations be \emph{product-oriented} \citep[see][]{Bybee1995,Gouskova.etal2015} -- that is, they reflect properties of the inflected (plural) form, not the base (singular); the generalizations in \sref{coranal} are instead \emph{source-oriented}, in that they reflect properties of the base (really, its underlying form).
...
% This approach has multiple issues, some empirical and some theoretical. Empirically, this method does not capture the preference for diminutive \ok{} among nouns with plural \pla{}. These would need to be grounded in a separate cloned transderivational constraint -- for example, \textsc{Ident}-[high]-OO, which prefers corresponding segments to agree in highness and thus prefers the suffix pair of plural [a] and diminutive \feat{ok} (both non-high) to plural [i] and diminutive \feat{ok} (high and non-high, respectively). We would also need to find constraint rankings in the universal set that prefer diminutives with each of \chik{} and \ik{} according to various phonological properties, as well as a more general constraint preferring \ok{} to capture words with \ok{} that have fixed stem stress and thus are penalized by \textsc{NoFlop-OO}. It is unclear whether it is realistic to find an appropriate constraint highlighting every combination of allomorph and phonological or morphological property favoring or disfavoring the productive extension of that allomorph to nonce words, as this method requires.

% The theoretical concern with this approach is, primarily, modularity. In order to make allomorph selection phonologically grounded (though not necessarily optimizing), \citet{Becker2009} places it in the phonology proper. % cite others
% Moreover, \textsc{NoFlop-OO}, as a transderivational constraint comparing multiple output forms, requires the generation (or retrieval) of multiple forms every time it is accessed -- at minimum, whenever a word with suffix or mobile stress forms a diminutive with \ok; this, in turn, requires phonology sending instructions back to the morphosyntax to construct the comparison forms. This violation of modularity is a bitter pill to swallow for many generative linguists; for those, this approach is a non-starter. % questionable empirical benefits
% % lexicalism

% Alternately, the target-oriented generalization can reside in a pattern-matching module like that described in \sref{coranal}, which does not implicate the generative phonology. This is the approach taken by \citet{Rebrus.etal2017}, who describe constraints that facilitate the selection of allomorphs that increase paradigm uniformity in Hungarian. In their analysis (in a non-generative framework that rejects UG), the constraints ``are not meant as OT constraints (which are part of UG), but as generalisations over (sets of) surface forms that speakers have memorised'' \citep[][167]{Rebrus.etal2017}.

% While target-oriented generalizations are necessary to describe patterns in allomorphy selection \citep[cf.][]{Bybee1995,Gouskova.etal2015}, paradigm uniformity constraints raise similar questions here as before: how does the speaker decide which forms need to be compared? For \citet{Rebrus.etal2017}, the relevant forms form a morphosyntactically coherent set, but this is not the case for Russian -- the speaker needs to pick out relevant forms from across a range of words built from the same base). Are these forms stored somehow, or are all relevant forms generated anew when needed to assign a diminutive feature to nonce words. This occurrence would be much rarer and, thus, less computationally costly than evaluating the relevant forms every time a diminutive is generated. It would also be less of an issue for modularity, as the pattern-matching module lies outside of phonology proper. As before, the paradigm stress uniformity approach does not explain why nouns with plural \pla{} take \ok{} more than other mobile-stress nouns. % does it matter?

% \ldots

% grounding approaches attempt to explain why certain patterns are learned - but here all the relevant ones are learned, so (from the perspective of the synchronic internal grammar) it could be coincidental

% \subsubsection{Abstract representation}\label{sec:abstrep}
% Many linguists % who?

% needed:
% phon/synt representation of diminutive selection
% phon/synt representation of plural -a
% these representations naturally cooccur
% non-diacritic marking of exceptions